...
const emailVerificationTemplate = ({ firstName, otpCode, verifyLink, expiresMinutes = 15 }) => ({
...
const orderStatusUpdateTemplate = ({ firstName, order, newStatus, message, trackingNumber, carrierName }) => {
  const statusMessages = {
    confirmed: { heading: 'Order Confirmed', body: 'Your order has been confirmed and payment has been received. We are now preparing your saddle for shipment.', badge: 'status-paid' },
    processing: { heading: 'Order in Processing', body: 'Your saddle is being carefully prepared for shipment. Our team is ensuring everything is perfect before it leaves our facility.', badge: 'status-pending' },
    shipped: { heading: 'Your Order Has Shipped!', body: 'Excellent news — your saddle is now on its way to you.', badge: 'status-shipped' },
    delivered: { heading: 'Order Delivered', body: 'Your order has been delivered. We hope you are delighted with your new saddle. Your 30-day trial period begins today.', badge: 'status-paid' },
    cancelled: { heading: 'Order Cancelled', body: message || 'Your order has been cancelled as requested.', badge: 'status-cancelled' },
    rejected: { heading: 'Order Update', body: message || 'We were unable to process your order. Please contact our team for assistance.', badge: 'status-cancelled' },
    refunded: { heading: 'Refund Processed', body: 'Your refund has been processed. Please allow 5-10 business days for it to appear in your account.', badge: 'status-paid' },
  };

  const statusInfo = statusMessages[newStatus] || { heading: 'Order Update', body: message || '', badge: 'status-pending' };
...
const cartAbandonmentTemplate = ({ firstName, emailNumber, cartItems, cartUrl, totalValue }) => {
  const messages = {
    1: {
...
const newsletterConfirmTemplate = ({ firstName, confirmLink, unsubscribeLink }) => ({
...
const newsletterBroadcastTemplate = ({ subject, htmlContent, previewText, unsubscribeLink }) => ({
  subject,
  html: baseTemplate(
    `${htmlContent}
    <hr class="email-divider" />
