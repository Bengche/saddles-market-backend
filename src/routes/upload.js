...
const { uploadProductImages, uploadSingleImage, uploadBlogImage } = require('../middleware/upload');
...
router.delete('/:cloudinaryId', adminOnly, async (req, res) => {
  try {
    ...
    await pool.query('DELETE FROM product_images WHERE cloudinary_id = $1', [publicId]);
