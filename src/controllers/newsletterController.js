...
const { newsletterConfirmTemplate, newsletterBroadcastTemplate } = require('../utils/emailTemplates');
...
const subscribe = async (req, res, next) => {
  try {
    ...
    const emailData = newsletterConfirmTemplate({ firstName: firstName || 'Equestrian', confirmLink, unsubscribeLink });
...
const confirmSubscription = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, message: 'Subscription confirmed! Welcome to the Saddles Market newsletter.' });
  } catch (err) {
    next(err);
  }
};
...
const sendCampaign = async (req, res, next) => {
  try {
    ...
    if (subscribers.rows.length === 0) {
      return res.json({ success: true, message: 'No active subscribers found.', sentTo: 0 });
    }
    ...
    for (const recipient of recipientsData) {
      ...
      const personalizedHtml = htmlContent.replace('{{first_name}}', recipient.first_name);
    ...
    res.json({
      ...
      data: { campaignId: campaign.rows[0].id, sentTo: subscribers.rows.length },
    });
  } catch (err) {
    next(err);
  }
...
const getSubscribers = async (req, res, next) => {
  try {
    ...
    const count = await pool.query(`SELECT COUNT(*) FROM newsletter_subscribers ${whereClause}`);
    const result = await pool.query(
...
module.exports = { subscribe, unsubscribe, confirmSubscription, sendCampaign, getSubscribers };
