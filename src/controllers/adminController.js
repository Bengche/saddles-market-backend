...
const getAdminDashboard = async (req, res, next) => {
  try {
    const [orders, revenue, users, products, pendingOrders, messages] = await Promise.all([
...
const getAdminUsers = async (req, res, next) => {
  try {
    ...
    const count = await pool.query(`SELECT COUNT(*) FROM users ${whereClause}`, params);
...
const adminCreateBlogPost = async (req, res, next) => {
  try {
    const {
      title, slug, excerpt, content, coverImage, coverImageAlt, category, tags,
      metaTitle, metaDescription, readingTime, isPublished,
    } = req.body;
    ...
    const result = await pool.query(
      ...
      [
        title, slug, excerpt, content, coverImage || null, coverImageAlt || title,
        req.user.id, req.user.first_name + ' ' + req.user.last_name,
        category || null, tags || null,
        metaTitle || title, metaDescription || excerpt, readingTime || 5,
        isPublished || false, isPublished ? new Date() : null,
      ]
    );
...
const adminUpdateBlogPost = async (req, res, next) => {
  try {
    ...
    const allowedFields = ['title', 'slug', 'excerpt', 'content', 'cover_image', 'cover_image_alt', 'category', 'tags', 'meta_title', 'meta_description', 'reading_time'];
    const updates = [];
    const values = [];
...
const approveReview = async (req, res, next) => {
  try {
    ...
    await pool.query('UPDATE product_reviews SET is_approved = $1 WHERE id = $2', [approved, id]);
...
const createCoupon = async (req, res, next) => {
  try {
    const { code, description, discountType, discountValue, minimumOrder, maximumDiscount, usageLimit, validUntil } = req.body;
    ...
    const result = await pool.query(
      ...
      [code.toUpperCase(), description, discountType, discountValue, minimumOrder || 0, maximumDiscount || null, usageLimit || null, validUntil || null]
    );
...
const adminGetProducts = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, products: result.rows, total: parseInt(count.rows[0].count) });
  } catch (err) { next(err); }
};
...
const adminPatchProduct = async (req, res, next) => {
  try {
    ...
    const updates = [];
    const values = [];
    if (is_featured !== undefined) { updates.push(`is_featured = $${values.length + 1}`); values.push(is_featured); }
    if (is_active !== undefined) { updates.push(`is_active = $${values.length + 1}`); values.push(is_active); }
    if (stock_quantity !== undefined) { updates.push(`stock_quantity = $${values.length + 1}`); values.push(stock_quantity); }
    if (!updates.length) return res.status(400).json({ success: false, message: 'No fields provided.' });
    values.push(id);
    const result = await pool.query(`UPDATE products SET ${updates.join(', ')} WHERE id = $${values.length} RETURNING *`, values);
...
const adminPatchOrder = async (req, res, next) => {
  try {
    ...
    if (tracking_number !== undefined) { updates.push(`tracking_number = $${values.length + 1}`); values.push(tracking_number); }
    if (!updates.length) return res.status(400).json({ success: false, message: 'No fields provided.' });
    values.push(id);
...
const adminPatchUser = async (req, res, next) => {
  try {
    const { id } = req.params;
    if (id === req.user.id) return res.status(400).json({ success: false, message: 'Cannot modify your own account.' });
    const { is_active, is_admin } = req.body;
    const updates = [];
    const values = [];
    if (is_active !== undefined) { updates.push(`is_active = $${values.length + 1}`); values.push(is_active); }
    if (is_admin !== undefined) { updates.push(`role = $${values.length + 1}`); values.push(is_admin ? 'admin' : 'customer'); }
    if (!updates.length) return res.status(400).json({ success: false, message: 'No fields provided.' });
    values.push(id);
...
const getNewsletterSubscribers = async (req, res, next) => {
  try {
    ...
    if (search) {
      params.push('%' + search + '%');
      conditions.push(`(email ILIKE $${params.length} OR first_name ILIKE $${params.length})`);
    }
    const where = conditions.length ? 'WHERE ' + conditions.join(' AND ') : '';
    const count = await pool.query(`SELECT COUNT(*) FROM newsletter_subscribers ${where}`, params);
    params.push(parseInt(limit), offset);
    const result = await pool.query(
    ...
    res.json({ success: true, subscribers: result.rows, total: parseInt(count.rows[0].count) });
  } catch (err) { next(err); }
};
...
const broadcastNewsletter = async (req, res, next) => {
  try {
    ...
    if (!subject || !content) return res.status(400).json({ success: false, message: 'Subject and content are required.' });
...
const getAdminStats = async (req, res, next) => {
  try {
    const [orders, revenue, users, products, pendingOrders, messages, newsletter] = await Promise.all([
      pool.query('SELECT COUNT(*) FROM orders'),
